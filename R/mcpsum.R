...
mcpsum <- function(cor, bfile, 
  ...
  if(gamma <= 1) stop("gamma should be bigger than 1")
  ...
  if(length(unique(chunks$chunks.blocks)) > 1) {
    if(is.null(cluster)) {
      results.list <- lapply(unique(chunks$chunks.blocks), function(i) {
        mcpsum(cor=cor[chunks$chunks==i], bfile=bfile, lambda=lambda, shrink=shrink, gamma=gamma, 
      ...
      results.list <- parallel::parLapplyLB(cluster, unique(chunks$chunks.blocks), function(i) {
        mcpsum(cor=Cor[chunks$chunks==i], bfile=Bfile, lambda=Lambda, 
  ...
  results <- runMcp(lambda[order], shrink, gamma, fileName=paste0(bfile,".bed"), 
