scadsum.pipeline <- function(cor, chr=NULL, pos=NULL, snp=NULL, 
                              A1=NULL, A2=NULL, 
                              ...
                              nomatch=FALSE, 
                              ...) {
  #' @title Run scadsum with standard pipeline
  #' @description The easy way to run scadsum 
  ...
  #' @param lambda to pass on to \code{\link{scadsum}}
  #' @param s A vector of s
  #' @param gamma A value of gamma
  #' @param destandardize Should coefficients from \code{\link{scadsum}} be 
  ...
  #' @param ... parameters to pass to \code{\link{scadsum}}
  #' 
  ...
  #' However, \bold{scadsum} with \code{s} < 1 is only run on SNPs that are 
  ...
  #' results from \code{scadsum} with s = 1 (soft-thresholding) 
  ...
  #' see the documentation for \code{keep} and \code{remove} in \code{\link{scadsum}} 
  #' for details. 
  #' @export
  #' 
  ...
  ### Get beta estimates from scadsum ###
  ...
  results <- list(beta=beta, test.extract=m.test$ref.extract, 
                  ...
                  exclude.ambiguous=exclude.ambiguous)
  #' @return A \code{scadsum.pipeline} object with the following elements
  #' \item{beta}{A list of scadsum coefficients: one list element for each \code{s}}
  ...
  #'  
  #'  ### Run scadsum using standard pipeline ### 
  #'  out <- scadsum.pipeline(cor=cor, chr=ss$Chr, pos=ss$Position, 
  #'                           A1=ss$A1, A2=ss$A2,
