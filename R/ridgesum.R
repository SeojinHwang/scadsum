...
#' \deqn{f(\beta)=\beta'R\beta - 2\beta'r + \lambda||\beta||_2^2}
#' where 
#' \deqn{R=(1-s)X'X/n + sI}
...
ridgesum <- function(cor, bfile, 
  ...
  if(trace > 0) {
    if(trace - floor(trace) > 0) {
      cat("Doing ridgesum on chunk", unique(chunks$chunks), "\n")
    } else {
  ...
  if(length(unique(chunks$chunks.blocks)) > 1) {
    if(is.null(cluster)) {
      results.list <- lapply(unique(chunks$chunks.blocks), function(i) {
        ridgesum(cor=cor[chunks$chunks==i], bfile=bfile, lambda=lambda, shrink=shrink, 
      ...
      results.list <- parallel::parLapplyLB(cluster, unique(chunks$chunks.blocks), function(i) {
        ridgesum(cor=Cor[chunks$chunks==i], bfile=Bfile, lambda=Lambda, 
 ...
 # print(4000-sum(extract2[[2]]))
  ...
  results <- runRidge(lambda[order], shrink, fileName=paste0(bfile,".bed"), 
  ...
  #' \item{fbeta}{\eqn{=\beta'R\beta - 2\beta'r + \lambda||\beta||_2^2}}
