mcpsum.pipeline <- function(cor, chr=NULL, pos=NULL, snp=NULL, 
                             A1=NULL, A2=NULL, 
                             ref.bfile=NULL, test.bfile=NULL, 
                             LDblocks=NULL, 
                             lambda=exp(seq(log(0.001), log(0.1), length.out=20)),
                             s=c(0.2, 0.5, 0.9),  # s should be 0<s<1
                             gamma=3,
                             destandardize=F, 
                             trace=1, 
                             ...
                             nomatch=FALSE, 
                             ...) {
  #' @title Run mcpsum with standard pipeline
  #' @description The easy way to run mcpsum 
  #' @param cor A vector of SNP-wise correlations with phenotype derived from summary statistics
  ...
  #' @param lambda to pass on to \code{\link{mcpsum}}
  #' @param s A vector of s
  #' @param gamma A value of gamma
  #' @param destandardize Should coefficients from \code{\link{mcpsum}} be 
  ...
  #' @param ... parameters to pass to \code{\link{mcpsum}}
  #' 
  ...
  #' However, \bold{mcpsum} with \code{s} < 1 is only run on SNPs that are 
  ...
  #' results from \code{mcpsum} with s = 1 (soft-thresholding) 
  ...
  #' see the documentation for \code{keep} and \code{remove} in \code{\link{mcpsum}} 
  #' for details. 
  #' @export
  #' 
  ...
  ### Get beta estimates from mcpsum ###
  ...
  if(length(s.minus.1) > 0) {
    ...
    ls <- lapply(s.minus.1, function(s) {
      if(trace) cat("s = ", s, "\n")
      mcpsum(cor=cor2, bfile=ref.bfile, 
              shrink=s, gamma=gamma, extract=ref.extract, lambda=lambda,
  ...
  results <- list(beta=beta, test.extract=m.test$ref.extract, 
                  ...
                  exclude.ambiguous=exclude.ambiguous)
  #' @return A \code{mcpsum.pipeline} object with the following elements
  #' \item{beta}{A list of mcpsum coefficients: one list element for each \code{s}}
  ...
  #' \item{lambda, s, gamma, keep.test, destandardized}{Information to pass on to \code{\link{validate.mcpsum.pipeline}} or \code{\link{pseudovalidate.mcpsum.pipeline}}}
  ...
  #'  
  #'  ### Run mcpsum using standard pipeline ### 
  #'  out <- mcpsum.pipeline(cor=cor, chr=ss$Chr, pos=ss$Position, 
  #'                           A1=ss$A1, A2=ss$A2,
