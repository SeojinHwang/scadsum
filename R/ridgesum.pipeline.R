ridgesum.pipeline <- function(cor, chr=NULL, pos=NULL, snp=NULL, 
                              A1=NULL, A2=NULL, 
                              ...
                              nomatch=FALSE, 
                              ...) {
  #' @title Run ridgesum with standard pipeline
  #' @description The easy way to run ridgesum 
  ...
  #' @param lambda to pass on to \code{\link{ridgesum}}
  #' @param s A vector of s
  #' @param destandardize Should coefficients from \code{\link{ridgesum}} be 
  ...
  #' @param ... parameters to pass to \code{\link{ridgesum}}
  #' 
  #' @details To run \bold{ridgesum} we assume as a minimum you have a vector of summary 
  ...
  #' If only \code{ref.bfile} is specified, only ridgesum coefficients are returned, 
  ...
  #' 
  #' \code{ridgesum.pipeline} only uses those SNPs that are consistently defined
  ...
  #' However, \bold{ridgesum} with \code{s} < 1 is only run on SNPs that are 
  #' common to all of \code{ref.bfile}, \code{test.bfile} and the 
  #' summary statistics. \bold{The ridgesum} coefficients for \code{s} < 1 are imputed with 
  #' results from \code{ridgesum} with s = 1 (soft-thresholding) 
  ...
  #' see the documentation for \code{keep} and \code{remove} in \code{\link{ridgesum}} 
  #' for details. 
  #' @export
  #' 
  ...
  ### Get beta estimates from ridgesum ###
  cor2 <- ss2$cor[sort(m.common$order)]
  ls <- list()
  if(length(s.minus.1) > 0) {
    if(trace) cat("Running ridgesum ...\n")
    ls <- lapply(s.minus.1, function(s) {
      if(trace) cat("s = ", s, "\n")
      ridgesum(cor=cor2, bfile=ref.bfile, 
  ...
  if(any(s == 1)) {
    if(trace) cat("Running ridgesum with s=1...\n")
  ...
  if(destandardize) {
    ...
    if(ref.equal.test) {
      if(any(xcl.test)) { # xcl.test => exclusive to test.bfile
        ...
        # sd should already be calculated at ridgesum
      }
    } else {
      sd <- sd.bfile(bfile = test.bfile, extract=m.test$ref.extract,  
    ...
    if(trace) cat("De-standardize ridgesum coefficients ...\n")
  ...
  results <- list(beta=beta, test.extract=m.test$ref.extract, 
                  ...
                  exclude.ambiguous=exclude.ambiguous)
  #' @return A \code{ridgesum.pipeline} object with the following elements
  #' \item{beta}{A list of ridgesum coefficients: one list element for each \code{s}}
