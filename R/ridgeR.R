#' @title Ridge using summary statistics
...
#' @keywords internal
ridgeR <- function(lambda1, lambda2=0, X, b, thr=1e-4,
  ...
  if(length(lambda2) > 1) {
    ...
    for(i in 1:nlambda2) {
      result <- ridgeR(lambda1, lambda2[i], X, b, thr,
                         trace, maxiter, x)
  ...
  for(i in 1:length(lambda1a)) {
    ...
    conv[i] <- repridge(lambda1a[i], lambda2, diag, X, b,thr,x,yhat, trace-1,maxiter,
    ...
    fbeta[i] <- loss[i] + sum(x^2)*lambda1a[i] + sum(x^2)*lambda2
  }
