#' @title Function to obtain SCAD estimates of a regression problem given summary statistics
#' and a reference panel
#' 
#' @details A function to find the minimum of \eqn{\beta} in  
#' \deqn{f(\beta)=\beta'R\beta - 2\beta'r + 2\rho_{scad}(\beta;\lambda,\gamma)}
#' where 
#' \deqn{R=(1-s)X'X/n + sI}
...
scadsum <- function(cor, bfile, 
  ...
  if(length(gamma) > 1) stop("Only 1 gamma parameter at a time.")
  if(gamma <= 2) stop("gamma should be bigger than 2")
  ...
  if(length(unique(chunks$chunks.blocks)) > 1) {
    if(is.null(cluster)) {
      results.list <- lapply(unique(chunks$chunks.blocks), function(i) {
        scadsum(cor=cor[chunks$chunks==i], bfile=bfile, lambda=lambda, shrink=shrink, gamma=gamma, 
      ...
      Cor <- cor; Bfile <- bfile; Lambda <- lambda; Shrink=shrink; Gamma=gamma; Thr <- thr; 
      ...
      results.list <- parallel::parLapplyLB(cluster, unique(chunks$chunks.blocks), function(i) {
        scadsum(cor=Cor[chunks$chunks==i], bfile=Bfile, lambda=Lambda, 
                 shrink=Shrink, gamma=Gamma, thr=Thr, init=Init[chunks$chunks==i], 
  ...
  results <- runScad(lambda[order], shrink, gamma, fileName=paste0(bfile,".bed"), 
